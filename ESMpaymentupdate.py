...
def showESMpaymentupdate():
    ...
    def updatedata():
        ...
        xf=int(e6.get())
        xg=int(e7.get())
        sql="update Payment set month='%s',deptid=%d,noofleaves=%d,payment=%d,tax=%d,netpay=%d where empid=%d"%(xb,xc,xd,xe,xf,xg,xa)
        cur.execute(sql)
        db.commit()
    ...
    def finddata():
        ...
        cur=db.cursor()
        xa=int(e1.get())
        sql="select empid, month, deptid, noofleaves, payment, tax, netpay from Payment where empid=%d"%(xa)
        cur.execute(sql)
        data=cur.fetchone()
        e2.insert(0,data[0])
        e3.insert(0,data[1])
