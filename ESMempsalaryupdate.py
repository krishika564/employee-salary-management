...
def showESMempsalaryupdate():
    ...
    def updatedata():
        ...
        xb=int(e2.get())
        xc=int(e3.get())
        sql="update Empsalary set salary=%d,monthsalary=%d where empid=%d"%(xb,xc,xa)
        cur.execute(sql)
        db.commit()
    ...
    def finddata():
        ...
        cur=db.cursor()
        xa=int(e1.get())
        sql="select salary,monthsalary from Empsalary where empid=%d"%(xa)
        cur.execute(sql)
        data=cur.fetchone()
        e2.insert(0,data[0])
        e3.insert(0,data[1])
    ...
    a2=Label(t,text='Salary',bg='#FFEDD1')
    a2.place(x=100,y=150)
    e2=Entry(t,width=30)
    e2.place(x=400,y=150)
    
    a3=Label(t,text='Month Salary',bg='#FFEDD1')
