...
def showESMpaymentsave():
    ...
    def savedata():
        if len(e1.get())==0 or len(e2.get())==0 or len(e3.get())==0 or len(e4.get())==0 or len(e5.get()) or len(e6.get()) or len(e7.get()):
            ...
            xf=int(e6.get())
            xg=int(e7.get())
            sql="insert into employee values (%d,'%s',%d,%d,%d,%d,%d)" %(xa,xb,xc,xd,xe,xf,xg)
            cur.execute(sql)
            db.commit()
